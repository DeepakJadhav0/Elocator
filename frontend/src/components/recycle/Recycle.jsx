...
const Recycle = () => {
  ...
  const userfacility = useSelector(state => state.userFacility)
  const navigate = useNavigate()

  useEffect(()=>{

    if(!userfacility || Object.keys(userfacility).length === 0){
  ...
  return (
    <div className="bg-white shadow-lg m-2 rounded-lg overflow-hidden hover:bg-gray-50 hover:shadow-xl transition-shadow duration-300 flex flex-col h-full">
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <button className="bg-green-500 text-white p-1 rounded-md hover:-translate-y-0.5 duration-300 transition-all hover:bg-green-600" onClick={()=>Nevigate(`/recycle/register?category=${itemName}`)}> Recycle {itemName} </button>
